...
module.exports = {
    ...
    removeActor: function (req, res) {
        Movie.findOne({_id: req.params.id}, function (err, movie) {
            ...
            if (!movie)
                return res.status(404).json();
            Actor.findOne({_id: req.params.actor_id}, function (err, actor) {
                if (err)
                    return res.status(400).json(err);
                if (!actor)
                    return res.status(404).json();
                movie.actors.pop(movie._id);
                movie.save(function (err) {
                    if (err)
                        return res.status(500).json(err);
                    res.json(movie);
    ...
    getSpecificMovies: function (req, res) {
        Movie.find({ year: {$lt: req.params.year1, $gt: req.params.year2}}).populate('actors').exec(function (err, movie) {
            if (err)
                return res.status(400).json(err);
            if (!movie)
                return res.status(404).json();
